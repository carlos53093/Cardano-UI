...
import { Blockfrost, Constr, Data, fromText, Lucid, toUnit } from "lucid/mod.ts";

import { Input } from "~/components/Input.tsx";
import { Button } from "~/components/Button.tsx";
import { AppliedValidators, applyParams, Validators, readMintValidator, readLockValidator, readDistroValidator } from "~/utils.ts";
    ...
    const createGiftCard = async (e: Event) => {
        ...
        try {
            ...
            const mintRedeemer = Data.to(
                new Constr(0, [BigInt(1), fromText(tokenName)])
            );
            ...
            const ownerPKH = lucid.utils.getAddressDetails(utxo.address).paymentCredential.hash;

            console.log(`${ownerPKH}`);
            const mint = await readMintValidator(ownerPKH);
            console.log(`${mint}`);
            const mintCS = lucid.utils.mintingPolicyToId(mint);
            console.log(`${mintCS}`);
            const lock = await readLockValidator(ownerPKH, mintCS);
            console.log(`${lock}`);
            const distro = await readDistroValidator(ownerPKH);
            console.log(`${distro}`);
            ...
            console.log(`${lAddress}, ${lDatum}, ${dAddress}, ${dDatum}`);

            // I don't pass this step
            const tx = await lucid!
                .newTx()
                .mintAssets(
                    {
                        [toUnit(mintCS, fromText(tokenName), 100)]: BigInt(1),
                        [toUnit(mintCS, fromText(tokenName), 444)]: BigInt(1000),
                    },
                    mintRedeemer
                )
